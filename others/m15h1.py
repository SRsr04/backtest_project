...
def load_and_prepare_data(candles_path, fvg_path, bos_path, h1_candles_path):
    """
    Завантажує, обробляє та ОБ'ЄДНУЄ дані для стратегії M15/H1 (без логіки БУ).
...
def run_optimized_backtest_m15h1_no_be(df_master, start_date, end_date, initial_balance, risk_per_trade, target_rr, params):
    """
    Виконує повний ОПТИМІЗОВАНИЙ бектест для стратегії M15/H1 БЕЗ логіки БУ.
    Повертає DataFrame з результатами угод та список значень equity curve.
    """
    logging.info("Починаємо оптимізований бектест M15/H1 (без БУ)...")
    print("Запускаємо оптимізований бектест M15/H1 (без БУ)...")
    ...
    print(f"Етап 1/2: Знайдено {len(fvg_events)} H1 FVG. Аналізуємо для пошуку сетапів...")
    for fvg_time, fvg_row in tqdm.tqdm(fvg_events.iterrows(), desc="Пошук сетапів (M15/H1)"):
    ...
    for setup in tqdm.tqdm(potential_setups, desc="Симуляція угод (M15/H1, без БУ)"):
        # Перевіряємо, чи немає активної угоди і чи поточний сетап не перетинається з попередньою закритою угодою
        # Додав перевірку, щоб уникнути одночасного відкриття кількох угод або накладання
        # Ця логіка може бути складною, якщо ви дозволяєте кілька одночасних угод.
        # Наразі припускаємо, що відкривається лише одна угода за раз.
        if active_trade is None and (not all_trades or setup['entry_time'] >= all_trades[-1]['exit_time']):
            ...
            if risk_abs == 0: continue # Уникнути ділення на нуль, якщо SL = Entry
            ...
            for _, candle in df_backtest[df_backtest.index > active_trade['entry_time']].iterrows():
                ...
                if outcome != 'pending':
                    ...
                    active_trade = None
                    break
        # Якщо є активна угода, або поточний сетап перетинається з попередньою закритою угодою
        # (і ми не дозволяємо паралельні угоди), то ми пропускаємо цей сетап.
        elif active_trade is not None:
             pass # Поточна угода ще відкрита, пропускаємо пошук нової
        else: # Якщо є перетин з попередньою закритою угодою, пропустити, або додати логіку для паралельної торгівлі
            pass # Наразі, не дозволяємо перетин угод, якщо попередню угоду ще не закрито.

    if not all_trades:
        # Якщо угод не було, equity_curve_values буде містити лише initial_balance
    ...
    results_dir, file_path = 'backtest_results', os.path.join('backtest_results', "m15h1.csv")
...
if __name__ == "__main__":
    ...
    try:
        ...
        kiev_tz = pytz.timezone('Europe/Kiev')
        # Важливо: встановлюйте часовий пояс для дат
        start_date = kiev_tz.localize(datetime(2020, 7, 19))
        end_date = kiev_tz.localize(datetime(2025, 7, 18, 23, 59, 59))
        
        initial_account_balance = 5000.0 # Винесено для зручності
        
        # Змінено: функція run_optimized_backtest_m15h1_no_be тепер повертає trade_results_df та equity_values
        trade_results_df, equity_values = run_optimized_backtest_m15h1_no_be(
            df_master, 
            start_date, 
            end_date, 
            initial_account_balance, # Передаємо початковий баланс
            1.0, 
            2.6, 
            {}
        )
        ...
        if not trade_results_df.empty:
            # Розрахунок просадки, використовуючи отриману криву капіталу
            ...
            # --- Вивід загальних результатів бектесту ---
            print("\n       BTC (M15/H1)")
            print("="*50)
            ...
            # Якщо угод немає, але крива капіталу все одно має початковий баланс,
            # розрахунок просадки все одно буде коректним (0%).
            if equity_values:
                max_dd_absolute, max_dd_percent = calculate_drawdown(equity_values)
