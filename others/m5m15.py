...
LOG_FILE_NAME = os.path.join(LOG_DIR, 'm5m15.log') 
...
def check_trade_close(trade, current_candle_data):
    ...
    Застосовує консервативний підхід: якщо SL та TP досягнуті в одній свічці,
    угода вважається закритою по стоп-лоссу.

    Args:
        trade (dict): Словник з деталями відкритої угоди.
    ...
    Returns:
        tuple: (outcome, exit_time, exit_price, pnl)
               outcome: 'win' (TP), 'loss' (SL), 'pending' (ще відкрита).
               exit_time (pd.Timestamp): Час закриття угоди.
               exit_price (float): Ціна закриття угоди.
               pnl (float): Прибуток/збиток від угоди.
    """
    ...
    logging.debug(f"Candle: High={high:.4f}, Low={low:.4f}")
    logging.debug(f"Trade {trade['entry_time'].strftime('%Y-%m-%d %H:%M')}: Entry={entry_price:.4f}, SL={stop_loss:.4f}, TP={take_profit:.4f}")
    ...
    if trade_type == 'long':
        ...
        if sl_hit and tp_hit:
            # Обидва досягнуті в одній свічці. Пріоритет SL для консервативності.
            outcome = 'loss'
            exit_price = stop_loss
            logging.debug(f"Лонг: ❌❌ Стоп-лосс ({stop_loss:.4f}) та Тейк-профіт ({take_profit:.4f}) досягнуто в одній свічці. Пріоритет SL.")
        elif sl_hit:
            outcome = 'loss'
            exit_price = stop_loss
            logging.debug(f"Лонг: ❌ Стоп-лосс спрацював на {current_time.strftime('%Y-%m-%d %H:%M')} за ціною {stop_loss:.4f}.")
        elif tp_hit:
            outcome = 'win'
        ...
        if sl_hit and tp_hit:
            ...
            outcome = 'loss'
            exit_price = stop_loss
            logging.debug(f"Шорт: ❌❌ Стоп-лосс ({stop_loss:.4f}) та Тейк-профіт ({take_profit:.4f}) досягнуто в одній свічці. Пріоритет SL.")
        elif sl_hit:
            outcome = 'loss'
            exit_price = stop_loss
            logging.debug(f"Шорт: ❌ Стоп-лосс спрацював на {current_time.strftime('%Y-%m-%d %H:%M')} за ціною {stop_loss:.4f}.")
        elif tp_hit:
            outcome = 'win'
            exit_price = take_profit
...
def load_and_prepare_data(candles_path, fvg_path, bos_path, higher_tf_candles_path):
    """
    Завантажує, обробляє та ОБ'ЄДНУЄ дані для стратегії M5/M15 без фільтрів.
    ...
    df_m15_candles = load_csv_to_datetime_index(higher_tf_candles_path)
    ...
    df_master = pd.merge_asof(df_candles, df_m15_candles, left_index=True, right_index=True, direction='backward')
    ...
    # --- КРОК 3: Розрахунок індикаторів та фінальна очистка ---
...
def run_optimized_backtest_no_filter(df_master, start_date, end_date, initial_balance, risk_per_trade, target_rr, params):
    """
    Виконує повний ОПТИМІЗОВАНИЙ бектест для стратегії M5/M15
    БЕЗ H1 фільтра та БЕЗ логіки переміщення в беззбиток.
    Повертає DataFrame з результатами угод та список значень equity curve.
    """
    logging.info("Починаємо оптимізований бектест M5/M15 (без фільтрів та БУ)...")
    print("Запускаємо оптимізований бектест M5/M15 (без фільтрів та БУ)...")
    ...
    print(f"Етап 1/2: Знайдено {len(fvg_events)} FVG. Аналізуємо для пошуку сетапів...")
    for fvg_time, fvg_row in tqdm.tqdm(fvg_events.iterrows(), desc="Пошук сетапів (без фільтрів)"):
    ...
    for setup in tqdm.tqdm(potential_setups, desc="Симуляція угод (без фільтрів, без БУ)"):
        if active_trade is None and (not all_trades or setup['entry_time'] >= all_trades[-1]['exit_time']):
            ...
            # comm, be_price = breakeven_info(setup['entry_price'], pos_size, setup['type']) # Закоментовано, якщо BE немає
            active_trade = {'type': setup['type'], 'entry_time': setup['entry_time'], 'entry_price': setup['entry_price'], 'stop_loss': setup['stop_loss'], 'take_profit': tp, 'position_size': pos_size, 'initial_stop_loss': setup['stop_loss'], 'fvg_type': setup['fvg_row']['fvg_type'], 'fvg_start_time': setup['fvg_row'].name, 'fvg_range': f"{setup['fvg_row']['fvg_min']:.1f}-{setup['fvg_row']['fvg_max']:.1f}", 'fractal_1_price': setup['f1_row']['high'] if setup['type'] == 'long' else setup['f1_row']['low'], 'fractal_2_price': setup['f2_row']['low'] if setup['type'] == 'long' else setup['f2_row']['high']} # Видалено BE-специфічні поля
            ...
            for _, candle in df_backtest[df_backtest.index > active_trade['entry_time']].iterrows():
                ...
                if outcome != 'pending':
                    ...
                    active_trade = None
                    break
                # Логіка БУ тут відсутня, як і було запрошено
    ...
    # Оновлений desired_order без BE-специфічних полів
    ...
    results_dir, file_path = 'backtest_results', os.path.join('backtest_results', "no_filter_m5m15.csv")
    os.makedirs(results_dir, exist_ok=True)
    results_df.to_csv(file_path, index=False, date_format='%Y-%m-%d %H:%M:%S')
    print(f"\nБектест (без фільтрів, без БУ) завершено. Знайдено {len(all_trades)} угод. Результати в '{file_path}'")
...
if __name__ == "__main__":
    ...
    try:
        # Припускаємо, що load_and_prepare_data_no_h4 (або аналогічна ваша функція) визначена.
        # Вона не повинна приймати H4_CANDLES_FILE, якщо сетап "no filter".
        df_master = load_and_prepare_data( # Використайте відповідну вашу функцію
            CANDLES_FILE, 
            FVG_FILE, 
            BOS_FILE, 
        ...
        trade_results_df, equity_values = run_optimized_backtest_no_filter(
            df_master, 
            start_date, 
            end_date, 
        ...
        if not trade_results_df.empty:
            ...
            # Тут немає BE, тому breakevens не потрібні
            
            total_pnl = trade_results_df['pnl'].sum()
            win_rate = (wins / total_trades * 100) if total_trades > 0 else 0
            
            print("\n         BTC (M5/M15, No Filter)")
            print("="*55)
