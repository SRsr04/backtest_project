...
LOG_FILE_NAME = os.path.join(LOG_DIR, 'm5m15_h1_be.log') # Додано timestamp до імені файлу
...
def run_optimized_backtest_h1_filter(df_master, start_date, end_date, initial_balance, risk_per_trade, target_rr, params):
    """
    Виконує повний ОПТИМІЗОВАНИЙ бектест для стратегії M5/M15 з H1 фільтром та логікою БУ.
    Повертає DataFrame з результатами угод та список значень equity curve.
    """
    logging.info("Починаємо оптимізований бектест M5/M15 (з H1 фільтром)...")
    print("Запускаємо оптимізований бектест M5/M15 (з H1 фільтром)...")
    ...
    for fvg_time, fvg_row in tqdm.tqdm(fvg_events.iterrows(), desc="Пошук сетапів з H1 фільтром"):
        
        if not is_trading_hour(fvg_time): continue

        h1_direction = fvg_row.get('h1_direction_h1')
        if h1_direction is None or h1_direction == '': # Додано перевірку на None/порожній рядок
            continue 
    ...
    for setup in tqdm.tqdm(potential_setups, desc="Симуляція угод"):
    ...
    results_dir, file_path = 'backtest_results', os.path.join('backtest_results', "h1_filter_be_m5m15.csv")
...
if __name__ == "__main__":
    ...
    try:
        ...
        trade_results_df, equity_values = run_optimized_backtest_h1_filter(
            df_master, 
            start_date, 
            end_date, 
