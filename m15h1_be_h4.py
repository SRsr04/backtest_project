...
LOG_FILE_NAME = 'eth/m15h1_be_h4.log'
...
def load_and_prepare_data(candles_path, fvg_path, bos_path, h1_candles_path, h4_candles_path):
    ...
    df_fvg = load_csv_to_datetime_index(fvg_path, 'time') # FVG тепер з H1
    df_fvg.rename(columns={'type': 'fvg_type', 'min': 'fvg_min', 'max': 'fvg_max', 'middle': 'fvg_middle'}, inplace=True)

    df_bos = load_csv_to_datetime_index(bos_path, 'bos_time_kiev') # BOS тепер з M15
    ...
    # Додаємо напрямок H4 свічки для фільтрації
    ...
    # Послідовно збагачуємо M15 дані
...
def run_optimized_backtest_m15h1_h4_filter(df_master, start_date, end_date, initial_balance, risk_per_trade, target_rr, params):
    ...
    for setup in tqdm.tqdm(potential_setups, desc="Симуляція угод (M15/H1/H4)"):
...
if __name__ == "__main__":
    ...
    H1_CANDLES_FILE = 'h1_candels.csv' # Додано H1_CANDLES_FILE

    try:
        # load_and_prepare_data має бути визначена у вашому файлі.
        # Вона повинна приймати всі 5 аргументів для цього сетапу.
        # Залежно від вашої реалізації load_and_prepare_data,
        # вона може бути load_and_prepare_data_with_h1 (якщо ви використовуєте H1 для фільтрації).
        ...
        if not trade_results_df.empty:
            ...
            print("\n         ЗАГАЛЬНІ РЕЗУЛЬТАТИ БЕКТЕСТУ (M5/M15 + H1 Filter + BE)")
            print("="*55)
