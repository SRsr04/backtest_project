...
LOG_FILE_NAME = os.path.join(LOG_DIR, 'm5m15_h1.log') # Додано timestamp до імені файлу
...
def load_and_prepare_data(candles_path, fvg_path, bos_path, higher_tf_candles_path, h1_candles_path):
    """
    Завантажує, обробляє та ОБ'ЄДНУЄ всі дані (включаючи H1) в єдиний "Майстер-DataFrame".
    ...
    fractal_high_prices = df_candles['high']
    fractal_low_prices = df_candles['low']
    ...
    df_master = pd.merge_asof(df_master, df_h1_candles, left_index=True, right_index=True, direction='backward')
    ...
    for col in ['fvg_type', 'bos_type', 'h1_direction_h1']: df_master[col].fillna('', inplace=True)
...
def check_trade_close(trade, current_candle_data):
    ...
    Повертає 'win', 'loss', 'breakeven' або 'pending'.
    """
    ...
    if trade_type == 'short':
        logging.debug(f"Шорт-угода {trade['entry_time']}: Candle High={high:.4f}, Low={low:.4f}, Close={close:.4f}")

    if trade_type == 'long':
        # 1. Перевірка на Тейк-Профіт
        if high >= take_profit:
            outcome = 'win'
            exit_price = take_profit
            logging.debug(f"Лонг: ✅ Тейк-профіт спрацював на {current_time} за ціною {take_profit:.4f}.")
        # 2. Перевірка на Стоп-Лосс (якщо не Тейк-Профіт)
        elif low <= stop_loss:
            outcome = 'loss'
        ...
        if low <= take_profit:
            outcome = 'win'
            exit_price = take_profit
            logging.debug(f"Шорт (Win Check): low ({low:.4f}) <= TP ({take_profit:.4f}) -> TRUE") 
            logging.debug(f"Шорт: ✅ Тейк-профіт спрацював на {current_time} за ціною {take_profit:.4f}.")
        # 2. Перевірка на Стоп-Лосс (якщо не Тейк-Профіт)
        elif high >= stop_loss:
            outcome = 'loss'
            exit_price = stop_loss
            logging.debug(f"Шорт (Loss Check): high ({high:.4f}) >= SL ({stop_loss:.4f}) -> TRUE")
...
def run_optimized_backtest_h1_filter_no_be(df_master, start_date, end_date, initial_balance, risk_per_trade, target_rr, params):
    """
    Виконує повний ОПТИМІЗОВАНИЙ бектест для стратегії M5/M15 з H1 фільтром,
    АЛЕ БЕЗ логіки переміщення в беззбиток.
    Повертає DataFrame з результатами угод та список значень equity curve.
    """
    logging.info("Починаємо оптимізований бектест M5/M15 (H1 фільтр, без БУ)...")
    print("Запускаємо оптимізований бектест M5/M15 (H1 фільтр, без БУ)...")
    ...
    print(f"Етап 1/2: Знайдено {len(fvg_events)} FVG. Аналізуємо з урахуванням H1 фільтру...")
    for fvg_time, fvg_row in tqdm.tqdm(fvg_events.iterrows(), desc="Пошук сетапів (H1 фільтр, без БУ)"):
        
        if not is_trading_hour(fvg_time): continue

        h1_direction = fvg_row.get('h1_direction_h1') # Використовуємо .get() на випадок відсутності
        if h1_direction is None or h1_direction == '':
            continue
        
        if (fvg_row['fvg_type'] == 'bullish' and h1_direction != 'bullish') or \
           (fvg_row['fvg_type'] == 'bearish' and h1_direction != 'bearish'):
            continue
    ...
    for setup in tqdm.tqdm(potential_setups, desc="Симуляція угод (без БУ)"):
        if active_trade is None and (not all_trades or setup['entry_time'] >= all_trades[-1]['exit_time']):
            ...
            # У версії без БУ ми не розраховуємо комісію та ціну БУ.
            ...
            for _, candle in df_backtest[df_backtest.index > active_trade['entry_time']].iterrows():
                # Логіка переміщення в БУ ВІДСУТНЯ
    ...
    results_dir, file_path = 'backtest_results', os.path.join('backtest_results', "h1_filter_m5m15.csv")
...
if __name__ == "__main__":
    ...
    try:
        ...
        trade_results_df, equity_values = run_optimized_backtest_h1_filter_no_be(
            df_master, 
            start_date, 
            end_date, 
